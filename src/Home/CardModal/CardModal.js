...
{ 
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        // Get item details
        const itemRes = await axios.get(`http://3.144.92.61/login/item/${itemId}`);
        ...
        // Get poster details
        const userRes = await axios.get(`http://3.144.92.61/login/user/${sid}`);
         setPoster(userRes.data);
  
        // Check if the logged-in user is the owner of the item
        const resultRes = await axios.get('http://3.144.92.61/login', {
          headers: {
    ...
    fetchData();
      ...
      {   
        ...
        axios.post("http://3.144.92.61/login/message",[{ "sid":sid, "message": message, "itemName": name} ], {
          headers: {
            withCredentials: true,
            cook: cookies.get("access-token")
        }})
      ...
      function deletePost()
      {
        ...
        axios.get("http://3.144.92.61/login/delete/"+itemId, { headers: {
            withCredentials: true,
            cook: cookies.get("access-token")
          }})
        .then(res=>{
          ...
          <div className="row g-3 modalCardDiv" >
            <div className="col-sm-7 detailCard">
              <div className="modalDivs">
                {image ? <img className="innerImg modalImage" src={`http://3.144.92.61/images/${image}`} alt={name}/>
                  : <div>Loading....</div>}
              </div>
